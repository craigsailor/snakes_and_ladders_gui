...
impl GameSquare {
    fn new(id: u32, x: f32, y: f32, scale: f32, color: u32, number: u32, font: &FontArc) -> Self {
        Self {
            id,
            x,
            y,
    ...
    fn draw(&self, pixmap: &mut Pixmap) {
        ...
        let text_size = self.size * 0.45; // Adjust text size relative to square size

        // Calculate text position (center it in the square)
        let text_x = self.x + self.size * 0.6;
        let text_y = self.y + self.size * 1.0;
        ...
        for c in text.chars() {
            ...
            if let Some(outlined) = self.font.outline_glyph(glyph) {
                ...
                current_x += self.font.h_advance_unscaled(glyph_id) * text_size
                    / self.font.units_per_em().unwrap() as f32;
            }
        }
...
impl Drawable for Arrow {
    fn draw(&self, pixmap: &mut Pixmap) {
        ...
        let arrow_size = self.thickness * 0.5; // Increased size for fatter arrowhead
    ...
    fn draw(&self, pixmap: &mut Pixmap) {
        ...
        for img_y in 0..img_height {
            for img_x in 0..img_width {
                ...
                let window_x = x + img_x as i32;
                let window_y = y + img_y as i32;

                // Check bounds
                if window_x >= 0
                    && window_y >= 0
                    && window_x < window_width as i32
                    && window_y < window_height as i32
                {
                    let buffer_index = (window_y as u32 * window_width + window_x as u32) as usize;
